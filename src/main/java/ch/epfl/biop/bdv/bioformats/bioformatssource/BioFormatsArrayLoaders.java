...
public class BioFormatsArrayLoaders {
    ...
    public static class BioFormatsUnsignedByteArrayLoader extends BioformatsArrayLoader implements CacheArrayLoader<VolatileByteArray> {
        ...
        public VolatileByteArray loadArray(int timepoint, int setup, int level, int[] dimensions, long[] min) throws InterruptedException {
            try {
                ...
                int nElements = (w * h * d);
                if (dimensions[2]==1) {
                    // Optimisation (maybe useful  ? should avoid an array allocation and the ByteBuffer overhead
                    return new VolatileByteArray(reader.openBytes(
                            switchZandC ? reader.getIndex(channel, minZ, timepoint) : reader.getIndex(minZ, channel, timepoint),
                            minX, minY, w, h), true);
                } else {
                    byte[] bytes = new byte[nElements];
                    int offset = 0;
                    for (int z = minZ; z < maxZ; z++) {
                        byte[] bytesCurrentPlane = reader.openBytes(
                                switchZandC ? reader.getIndex(channel, z, timepoint) : reader.getIndex(z, channel, timepoint),
                                minX, minY, w, h);
    ...
    public static class BioFormatsUnsignedShortArrayLoader extends BioformatsArrayLoader implements CacheArrayLoader<VolatileShortArray> {
    ...
    public static class BioFormatsFloatArrayLoader extends BioformatsArrayLoader implements CacheArrayLoader<VolatileFloatArray> {
    ...
    public static class BioFormatsRGBArrayLoader extends BioformatsArrayLoader implements CacheArrayLoader<VolatileIntArray> {

        public BioFormatsRGBArrayLoader(ReaderPool readerPool, int series, int channel, boolean switchZandC) {
            super(readerPool, series, channel, switchZandC);
        }

        // Annoying because bioformats returns 3 bytes, while imglib2 requires ARGB, so 4 bytes
        @Override
        public VolatileIntArray loadArray(int timepoint, int setup, int level, int[] dimensions, long[] min) throws InterruptedException {
            try {
                ...
                for (int i = 0; i<nElements; i++) {
                    ints[i] = ((0xff) << 24 ) | ((bytes[idxPx] & 0xff) << 16 ) | ((bytes[idxPx + 1] & 0xff) << 8) | (bytes[idxPx+2] & 0xff);
                    idxPx+=3;
                }
    ...
    public static class BioFormatsIntArrayLoader extends BioformatsArrayLoader implements CacheArrayLoader<VolatileIntArray> {
